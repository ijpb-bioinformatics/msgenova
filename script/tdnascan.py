...
def align2genome(R1,R2,genome,outfile,thread,directory):
    ...
    cmd0 = "ln -s "+genome+" "+directory+"/"+genome_name
    print "cmd0"
    print cmd0
    print "directory"
    print directory
    ...
    print genome
    # ln on suffix
    for suffix in suff_bwa:
        if not os.path.islink(directory+"/"+genome_name+"."+suffix):
            cmd02 = "ln -s "+genome+"."+suffix+" "+directory+"/"+genome_name+"."+suffix
            os.system(cmd02)
...
def fileSplit(tdnaSAM,tmp_dir,install_dir):
    ...
    cmd2 = "python2.7 "+install_dir+"/FileSplitter.py "+ tmp_sam +" 10000"
    #print cmd2
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', action='store',dest='install_dir',help="installation directory")
    parser.add_argument('-d', action='store', dest='dir_out', help="output directory")
...
#            raise
    ...
    for f in files:
        os.remove(f)
    fileSplit(tdnaSAM,tmp_dir,install_dir)
