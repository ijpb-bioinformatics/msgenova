...
workspace<-"/home/gabriele.adam/Documents/Msgenova/sorties_report/results"
...
print(vector)
# read sample file
sample_sheet<-read.table(paste0(workspace,"/genome/sample_sheet"),sep="\t",header=TRUE)
```
...
presence_of_insertion<-TRUE
}else{
  presence_of_insertion<-FALSE
}
```

`r ifelse(presence_of_insertion,"## By vector, how many insertions are there by chromosome ?{.tabset .tabset-fade}","")`

```{r barplot_snp, results='asis',eval=presence_of_insertion}
...
for (v in vector){
  ...
  if(nrow(sub_table)> 0){
    cat("###",v,"\n")
  g<- ggplot(sub_table,aes(x=Chr,y=nbr_inster_by_chr,fill=sample))+geom_bar(stat="identity",position=position_dodge())+ylab("Numbre of insertion by chromosome")+xlab("Chromosome")+ggtitle(paste0("Number of in sertion of vector ",v," by chromosome"))+theme_bw()
  print(g)
  }
  cat('\n\n')
}
...
# Does several samples have the same insertions ?
```{r same_insertions, eval=presence_of_insertion}
essai<-search_identical_insertion(final_tab,5000)
DT::datatable(essai)
```

`r ifelse(vector_calling,"# Barplot representation of the insertions in each sample","")`

```{r plot_by_chr_tuto, eval=presence_of_insertion, echo = presence_of_insertion}
...
```{r no_insertions, eval=presence_of_insertion}
